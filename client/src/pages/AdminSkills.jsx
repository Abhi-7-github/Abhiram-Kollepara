...
export default function AdminSkills() {
  ...
  const refresh = async () => {
    ...
    try {
      ...
      setSkills([]);
      setStatus({ state: 'error', error: err?.message || 'Failed to load skills' });
    }
  };
      ...
      <div className="scanlines min-h-dvh flex flex-col">
        ...
        <section className="mx-auto w-full max-w-4xl flex-1 px-4 py-10 space-y-6">
          <div className="rounded-2xl border border-green-500/15 bg-black/40 p-6">
            <div className="font-mono text-sm text-green-200">skills editor</div>
          ...
          <AdminContentManager skills={skills} projects={[]} onRefresh={refresh} forceShow view="skills" />
        </section>
      </div>
    </m.main>
  );
